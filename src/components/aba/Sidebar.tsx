...
const items: { key: TabKey; label: string; icon: React.ComponentType<{ className?: string }> }[] = [
  ...
  { key: "logger", label: "ABC Logger", icon: ClipboardList },
  { key: "simulator", label: "FA Simulator", icon: FlaskConical },
    ...
    <>
      ...
      <aside className="hidden md:flex w-64 shrink-0 flex-col border-r border-border bg-card/60 backdrop-blur px-4 py-6 gap-2 sticky top-0 h-screen">
        <div className="flex items-center gap-2 px-2 mb-6">
          ...
          <div>
            <div className="text-sm font-semibold leading-tight">ABC Tracker</div>
            <div className="text-xs text-muted-foreground">FA Simulator</div>
          </div>
        </div>
        ...
        <div className="mt-auto text-[11px] text-muted-foreground px-2 leading-relaxed">
          For clinical training & research use. Data stored locally.
        </div>
      </aside>
